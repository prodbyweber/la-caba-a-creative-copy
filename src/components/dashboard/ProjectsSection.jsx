...
import { Plus, FolderOpen, Music2, Pencil, Globe, Lock } from "lucide-react";
                          ...
                          <div className="relative aspect-square rounded-lg overflow-hidden bg-gradient-to-br from-white/10 to-white/5 mb-1.5">
                            ...
                            <div className="absolute bottom-1 left-1 flex items-center gap-1">
                              <button onClick={(e) => handleTogglePublic(e, project)}
                                className={`w-5 h-5 rounded-md flex items-center justify-center transition-colors ${project.is_public ? "bg-emerald-500/30" : "bg-black/60"}`}>
                  ...
                  return (
                    <div key={project.id} className="flex-[0_0_150px] sm:flex-[0_0_175px]">
                      <motion.div
                        ...
                        className="group/proj bg-white/5 rounded-xl p-2 border border-white/5 hover:border-emerald-500/30 transition-all h-full"
                      >
                        <div className="relative aspect-square rounded-lg bg-gradient-to-br from-emerald-500/20 to-purple-500/20 mb-2 overflow-hidden cursor-pointer">
                          <Link to={createPageUrl(`ProjectDetail?id=${project.id}`)}>
                            {project.cover_url ? (
                              <img src={project.cover_url} alt={project.title} className="w-full h-full object-cover" />
                            ) : projectTracks[0]?.cover_url ? (
                          ...
                          <div className="absolute bottom-1.5 left-1.5 flex items-center gap-1 opacity-0 group-hover/proj:opacity-100 transition-opacity">
                            <button onClick={(e) => handleTogglePublic(e, project)}
                              title={project.is_public ? "Público" : "Privado"}
                              className={`w-6 h-6 rounded-md flex items-center justify-center transition-colors ${project.is_public ? "bg-emerald-500/30 hover:bg-emerald-500/40" : "bg-black/70 hover:bg-black/90"}`}>
                              {project.is_public ? <Globe className="w-3 h-3 text-emerald-400" /> : <Lock className="w-3 h-3 text-white/40" />}
                            </button>
                            <button onClick={(e) => handleEdit(e, project)}
                              className="w-6 h-6 rounded-md bg-black/70 hover:bg-black/90 flex items-center justify-center transition-colors">
                        ...
                        <Link to={createPageUrl(`ProjectDetail?id=${project.id}`)}>
                          <div className="space-y-0.5 px-0.5">
                            <h4 className="font-semibold text-xs text-white group-hover/proj:text-emerald-400 transition-colors truncate">{project.title}</h4>
