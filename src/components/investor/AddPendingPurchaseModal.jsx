...
export default function AddPendingPurchaseModal({ isOpen, onClose, purchase = null }) {
  ...
  const mutation = useMutation({
    mutationFn: (data) => purchase 
      ? base44.entities.PendingPurchase.update(purchase.id, data)
      : base44.entities.PendingPurchase.create(data),
    onSuccess: () => {
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <motion.div
          ...
          className="bg-[#1a1a1c] rounded-2xl w-full max-w-md p-6 border border-white/10"
        >
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-white">
              {purchase ? "Editar Compra" : "Agregar Compra Pendiente"}
            </h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-[#0f1011] border border-white/10 rounded-lg focus:ring-2 focus:ring-orange-500 text-white"
              />
            </div>
