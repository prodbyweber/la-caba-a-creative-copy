                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="space-y-4">
                    <motion.div
                      ...
                      className="w-20 h-20 rounded-2xl overflow-hidden border-3 border-emerald-500/50 bg-gradient-to-br from-emerald-500 to-emerald-700 mx-auto shadow-xl"
                    >
                      <img 
                    ...
                    <div className="text-center">
                      <div className="flex items-center justify-center gap-2 mb-1">
                        <h3 className="text-2xl font-black text-white uppercase">JLY</h3>
                        <Verified className="w-5 h-5 text-emerald-500" />
                      </div>
                      <p className="text-sm text-gray-400">Urban / Catalan Trap</p>
                      <p className="text-xs text-gray-500 mt-1">Barcelona, España</p>
                    </div>
                  </div>
                  ...
                  <motion.div
                    ...
                    className="p-4 rounded-xl bg-gradient-to-b from-emerald-600/20 to-emerald-800/20 border border-emerald-500/30"
                  >
                    <div className="text-center">
                      <div className="text-4xl font-black text-emerald-400 mb-1">92</div>
                      <div className="text-xs font-bold text-emerald-200 uppercase tracking-wide">Artist Score</div>
                    </div>
                  </motion.div>
                  ...
                  <div className="space-y-2">
                    <motion.div
                      whileHover={{ x: 4 }}
                      className="p-3 rounded-lg bg-white/5 border border-white/10 flex items-center justify-between hover:bg-white/10 transition-colors cursor-default"
                    >
                      <div className="flex items-center gap-2">
                        <Music2 className="w-4 h-4 text-emerald-400" />
                        <span className="text-sm text-gray-300">Tracks Publicados</span>
                      </div>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-300">Horas Studio</span>
                      </div>
                      <span className="font-bold text-orange-400">48h</span>
                    </motion.div>
                  </div>
                </div>
              </motion.div>
              ...
              >
                <div className="p-6 space-y-4">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-bold text-white">Tracks Recientes</h3>
                  </div>

                  <div className="space-y-2">
                    {[
                      { name: "Amanecer", badge: "Dolby Atmos", color: "from-purple-500/20 to-pink-500/20", textColor: "text-purple-400", badgeColor: "bg-orange-500/10 text-orange-400" },
                      ...
                      <motion.div
                        ...
                        className="p-3 rounded-lg bg-white/5 border border-white/10 hover:border-purple-500/30 hover:bg-white/10 transition-all cursor-pointer group"
                      >
                        <div className="flex items-center gap-3">
                          <motion.div
                            animate={hoveredTrack === idx ? { scale: 1.1 } : { scale: 1 }}
                            className={`w-8 h-8 rounded bg-gradient-to-br ${track.color} flex items-center justify-center flex-shrink-0`}
                          >
                            <Play className={`w-3 h-3 ${track.textColor}`} />
                          </motion.div>
                          <div className="flex-1 min-w-0">
                            <div className="text-sm text-white font-medium truncate group-hover:text-purple-300 transition-colors">{track.name}</div>
                          </div>
                          <motion.div
                            animate={hoveredTrack === idx ? { opacity: 1, x: 0 } : { opacity: 0.6, x: -4 }}
                            className={`text-xs px-2 py-1 rounded ${track.badgeColor} font-bold whitespace-nowrap`}
                          >
                            {track.badge}
                          </motion.div>
                        </div>
              ...
              >
                <div className="p-6 space-y-4">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-lg font-bold text-white">Próximas Sesiones</h3>
                    <motion.div
                      animate={{ scale: [1, 1.2, 1] }}
                      transition={{ duration: 2, repeat: Infinity }}
                      className="w-2 h-2 rounded-full bg-orange-400 ml-auto"
                    />
                  </div>

                  <div className="space-y-3">
                    {[
                      { day: "Lun 20:00", task: "Grabación de Voz", icon: "🎤", color: "emerald" },
                      { day: "Mié 18:00", task: "Sesión de Mix", icon: "🎛️", color: "purple" },
                      { day: "Vie 15:00", task: "Master Check", icon: "✨", color: "orange" }
                    ].map((session, idx) => (
                      <motion.div
                        key={idx}
                        whileHover={{ x: 4, scale: 1.02 }}
                        className="p-3 rounded-lg bg-gradient-to-r from-white/5 to-white/0 border border-white/10 hover:border-orange-500/30 hover:bg-white/10 transition-all group"
                      >
                        <div className="flex items-center gap-3">
                          <div className="text-2xl flex-shrink-0">{session.icon}</div>
                          <div className="flex-1 min-w-0">
                            <div className={`text-xs font-bold text-${session.color}-400 uppercase tracking-wide`}>{session.day}</div>
                            <div className="text-sm text-white font-medium group-hover:text-orange-300 transition-colors">{session.task}</div>
                          </div>
                          <motion.div
                            animate={{ x: [0, 4, 0] }}
                            transition={{ duration: 2, repeat: Infinity }}
                  ...
                  >
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      >
                        ...
                        <div className="text-[10px] text-gray-500">Productividad</div>
                      </motion.div>
                      <motion.div
                        whileHover={{ scale: 1.05 }}
                        className="p-2 rounded-lg bg-white/5 border border-white/10 text-center hover:bg-white/10 transition-colors"
                      >
                        ...
                        <div className="text-[10px] text-gray-500">Este Mes</div>
                      </motion.div>
                    </div>
                  </motion.div>
                </div>
