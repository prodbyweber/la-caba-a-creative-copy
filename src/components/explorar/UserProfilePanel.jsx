            ...
            <>
              ...
              <div className="relative pb-4">
                ...
                <div className="relative px-5 -mt-14 flex flex-col items-center text-center">
                  ...
                  {/* Public profile link — prominent, with avatar + verified badge */}
                  {userProfile?.username && (
                    <a
                      href={`https://cabanacreative.es/${userProfile.username}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full flex items-center gap-3 px-3.5 py-3 rounded-2xl border border-orange-500/25 bg-orange-500/[0.07] hover:bg-orange-500/[0.12] hover:border-orange-500/50 transition-all group mb-3"
                      onClick={e => e.stopPropagation()}
                    >
                      {/* Avatar correspondiente a la cuenta */}
                      <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-orange-500/40 flex-shrink-0 flex items-center justify-center bg-[#1a1a1a]">
                        {avatarUrl ? (
                          <img src={avatarUrl} alt={displayName} className="w-full h-full object-cover" />
                        ) : (
                          <span className="text-sm font-black text-orange-400/60">{initials}</span>
                        )}
                      </div>
                      <div className="flex-1 min-w-0 text-left">
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs font-semibold text-orange-400 group-hover:text-orange-300 transition-colors truncate">
                            {linkedArtist?.stageName || userProfile?.artist_name || displayName}
                          </span>
