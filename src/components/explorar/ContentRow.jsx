...
import { ChevronLeft, ChevronRight, Music2, Play, Pause, X, ChevronDown, Heart, Bookmark } from "lucide-react";
...
// ── Credits Modal ──────────────────────────────────────────────────────────
function CreditsModal({ item: initialItem, playing, onClose, onToggleAudio, currentUser, allItems }) {
  ...
  return createPortal(
    <>
      ...
      {/* YouTube player — dentro del mismo portal, z-index mayor */}
      <AnimatePresence>
        {showYT && ytId && (
          <YoutubeModal
            ytId={ytId}
...
function ContentCard({ item, onClick, currentUser, allItems }) {
  ...
  const openYT = (e) => {
    if (e) e.stopPropagation();
    setShowYTModal(true);
    explorar?.setCardModalOpen(true);
  };

  const closeYT = () => {
    setShowYTModal(false);
    explorar?.setCardModalOpen(false);
  };
  ...
  return (
    <>
      {/* YouTube modal directo desde la tarjeta */}
      <AnimatePresence>
        {showYTModal && ytId && (
          <YoutubeModal
            ytId={ytId}
