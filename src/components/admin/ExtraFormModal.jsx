...
// CRUD de extras de reserva.
export default function ExtraFormModal({ extra, onClose }) {
  const queryClient = useQueryClient();
  const isEdit = !!extra;
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ name: "", description: "", price: 0, type: "simple", active: true, sort_order: 0 });

  useEffect(() => { if (extra) setForm({ ...extra, price: extra.price || 0, sort_order: extra.sort_order || 0 }); }, [extra]);
  const set = (f) => (e) => setForm(s => ({ ...s, [f]: e.target.type === "checkbox" ? e.target.checked : e.target.value }));

  const handleSave = async () => {
    if (!form.name?.trim()) { alert("Nombre obligatorio"); return; }
    setSaving(true);
    try {
      const payload = { ...form, price: Number(form.price) || 0, sort_order: Number(form.sort_order) || 0 };
      if (isEdit) await base44.entities.ReservationExtra.update(extra.id, payload);
      else await base44.entities.ReservationExtra.create(payload);
      queryClient.invalidateQueries({ queryKey: ["reservation-extras"] });
      onClose();
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96 }}
          ...
          <div className="p-5 border-b border-white/[0.07] flex items-center justify-between">
            <h3 className="text-base font-bold text-white">{isEdit ? "Editar extra" : "Nuevo extra"}</h3>
          ...
          <div className="p-5 space-y-4">
            ...
            <div><label className="lbl">Descripción</label><input value={form.description} onChange={set("description")} className="inp" /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="lbl">Precio (€)</label><input type="number" value={form.price} onChange={e => setForm(s => ({ ...s, price: Number(e.target.value) || 0 }))} className="inp" /></div>
              <div><label className="lbl">Tipo</label>
                <select value={form.type} onChange={set("type")} className="inp">
                  <option value="simple">Simple (fijo)</option><option value="beat">Beat (catálogo)</option><option value="hours_extra">Horas extra</option>
                </select>
              </div>
            </div>
