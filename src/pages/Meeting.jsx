...
export default function Meeting() {
  ...
  const videoSrc = isVideo(cfg?.hero_video_url) ? cfg.hero_video_url
    : isVideo(cfg?.hero_banner_1_image) ? cfg.hero_banner_1_image
    : null;

  const vidRef = useAutoPlay(videoSrc);
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        {videoSrc ? (
          <video
            ...
            style={{ pointerEvents: "none", filter: "brightness(0.3) saturate(0.6) blur(2px)" }}
          />
        ) : cfg?.hero_banner_1_image ? (
          <img
            src={cfg.hero_banner_1_image}
            alt=""
            className="absolute inset-0 w-full h-full object-cover"
        ...
        <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(8,8,8,0.7) 0%, rgba(8,8,8,0.3) 35%, rgba(8,8,8,0.6) 70%, rgba(8,8,8,0.9) 100%)" }} />
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 90% 60% at 50% 50%, transparent 0%, rgba(8,8,8,0.5) 100%)" }} />
        <div className="absolute top-0 left-0 right-0 h-[3px]" style={{ background: "rgba(8,8,8,0.95)" }} />
        <div className="absolute bottom-0 left-0 right-0 h-[3px]" style={{ background: "rgba(8,8,8,0.95)" }} />
      </div>
      ...
      {isAdmin && (
        ...
        >
          <Link
            to="/AdminDashboard"
            className="flex items-center gap-2 text-white/30 hover:text-white/70 transition-colors"
      ...
      >
        ...
        >
          <div
            ...
            data-url="https://calendly.com/hola-cabanacreative/creadores?primary_color=ff4300&hide_gdpr_banner=1&background_color=080808&text_color=f0ede8"
