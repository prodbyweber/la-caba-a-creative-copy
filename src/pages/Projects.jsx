        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="relative mb-4">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-lg py-2 pl-10 pr-4 text-sm placeholder:text-gray-500 focus:outline-none focus:border-emerald-500/50"
            />
          </div>
                ...
                return (
                  <Link key={project.id} to={createPageUrl(`ProjectDetail?id=${project.id}`)}>
                    <motion.div
                      ...
                      className={`bg-white/5 rounded-xl p-4 border transition-all cursor-pointer group hover:border-emerald-500/30 ${
                        isOverdue ? 'border-red-500/20' : 'border-white/5'
                      }`}
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-white text-sm mb-1 group-hover:text-emerald-400 transition-colors truncate">
                            {project.title}
                          </h3>
                          {artist && (
                            <p className="text-xs text-gray-500 truncate">{artist.stageName}</p>
                          )}
                        </div>
                        {isOverdue && (
                      ...
                      <div className="flex gap-1.5 mb-3 flex-wrap">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-medium border ${statusColors[project.status]}`}>
                          {project.status}
                        </span>
